...
# def reserve_memory_on_device(device_index):
...
#
#     # 占用约90%的显存
#     reserved_memory = int(total_memory * 0.46) / 4  # 假设每个float32占用4字节
#
#     # 创建一个大的Tensor占用相应的显存
#     big_tensor = torch.empty(int(reserved_memory), dtype=torch.float32, device=device)
#     big_tensor.fill_(1)  # 用某个数值填充Tensor，确保显存被实际分配
#
#     return big_tensor
#
#
# # 预留显存
# big_tensors = []
# for i in range(4):  # 根据设置的GPU数量进行循环
#     big_tensors.append(reserve_memory_on_device(i))

# # cxg2
# data_root = {'ImageNet': '/data0/ImageNet_LT',
#              'Places': '/datasets01_101/Places365/041019',
#              'iNaturalist18': '/data0/iNaturalist18',
#              'Cifar': '/data0'}
# .133
data_root = {'ImageNet': '/data/pengru/datasets/ImageNet_LT',
             'iNaturalist18': '/data/pengru/datasets/iNaturalist18'}
