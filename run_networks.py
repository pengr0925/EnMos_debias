...
from tools.boundary_energy import cal_boundary_energy
from tools.utils import get_label_freq_bias
...
class model():
    ...
    def __init__(self, config, data, test=False):
        ...
        self.writer_acc = SummaryWriter(log_dir=os.path.join('./logs',
                                                             self.training_opt['dataset'],
                                                             'tensorboard_acc'))
    ...
    def init_criterions(self):
        ...
        for key, val in criterion_defs.items():
            ...
            self.criterions[key] = source_import(def_file).create_loss(*loss_args, self.data['train'].dataset.labels).cuda()
    ...
    def batch_forward (self, inputs, labels=None, centroids=False, feature_ext=False, phase='train'):
        ...
        if not feature_ext:
            ...
            self.logits, self.mixing_weights = self.networks['classifier'](self.features, labels, centroids_)
    ...
    def batch_backward(self):
        ...
        for key in self.networks.keys():
            torch.nn.utils.clip_grad_norm_(self.networks[key].parameters(), max_norm=5)
    ...
    def batch_loss(self, labels, adaptive_marg_coef):
        self.loss = 0

        # First, apply performance loss
        if 'PerformanceLoss' in self.criterions.keys():
            self.loss_perf = self.criterions['PerformanceLoss'](self.logits, labels, self.mixing_weights)
    ...
    def train(self):
        ...
        for epoch in range(1, end_epoch + 1):
            ...
            # adaptive_marg_coef = 1 - torch.pow(torch.tensor(epoch / end_epoch), 2)
            # adaptive_marg_coef = torch.pow(torch.tensor(epoch / end_epoch), 2)
            adaptive_marg_coef = 1 - torch.cos((epoch / end_epoch) * torch.tensor(torch.pi / 2))
            ...
            for step, (inputs, labels, indexes) in enumerate(self.data['train']):
                ...
                with torch.set_grad_enabled(True):
                    ...
                    self.batch_forward(inputs, labels, 
                                       ...
                                       phase='train')
                    self.batch_loss(labels, adaptive_marg_coef)
                    self.batch_backward()
                    ...
                    if self.mixing_weights is None:
                        ...
                        probs = self.criterions['PerformanceLoss'].calculate_probs(self.logits, self.mixing_weights)
            ...
            self.writer_acc.add_scalars(
                main_tag='acc--{}'.format(self.training_opt['log_dir'].split('/')[-2]),
                tag_scalar_dict={
                    '{}_train_acc_total'.format(self.training_opt['log_dir'].split('/')[-1]): rsls_train['train_all'],
                    '{}_test_acc_total'.format(self.training_opt['log_dir'].split('/')[-1]): rsls_eval['val_all'],
                },
                global_step=epoch
            )
    ...
    def eval(self, phase='val', openset=False, save_feat=False):
        ...
        self.total_probs = torch.empty((0, self.training_opt['num_classes'])).cuda()
            ...
            with torch.set_grad_enabled(False):
                ...
                if not get_feat_only:
                    if self.mixing_weights is None:
                        ...
                        mixed_probs = self.criterions['PerformanceLoss'].calculate_probs(self.logits, self.mixing_weights)
                        self.total_probs = torch.cat((self.total_probs, mixed_probs))
                        self.total_labels = torch.cat((self.total_labels, labels))
                        self.total_paths = np.concatenate((self.total_paths, paths))
        ...
        if self.mixing_weights is None:
            ...
            probs, preds = self.total_probs.max(dim=1)

        # if phase == 'test':
        #     class_bias = get_label_freq_bias(self.data['train'].dataset.labels)
        #     cal_boundary_energy(self.total_logits.cpu(), self.total_labels.cpu(), class_bias, preds.cpu())
